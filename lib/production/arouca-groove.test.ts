...
import { AROUCA_GROOVE, AROUCA_GROOVE_STOPS, CORNER_SHOP_ACTIVITIES, STORYPATH } from "./arouca-groove";

describe("Arouca Groove definition", () => {
  it("belongs to StoryPath and contains the exact ordered 18 approved stops", () => {
    expect(STORYPATH.name).toBe("StoryPath");
    expect(AROUCA_GROOVE.name).toBe("Arouca Groove");
    expect(AROUCA_GROOVE.countryId).toBe(STORYPATH.id);
