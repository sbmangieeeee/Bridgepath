...
The Teacher Lesson preserves the 1672×941 classroom plate at its native aspect ratio and renders the verified `ms-leela-maharaj-transparent.png` as a separate full-body layer. The visible figure is grounded from the scene bottom and kept left of the accessible lesson copy using the named CSS anchors `--teacher-scene-left`, `--teacher-scene-bottom` and `--teacher-scene-height`. Desktop uses a maximum 723 px figure height. At 844×390 the proportional classroom plate is 370 px high, Ms. Leela is 292 px high on the same y=370 ground plane, the measured whiteboard frame ends at y=329, and the separate 44 px HTML navigation row begins at y=337. Mobile portrait at up to 600 px wide does not compress the scene; the reusable gameplay shell presents an accessible rotate-device prompt and leaves the mounted lesson state intact. The source artwork, whiteboard, lesson text, controls and HUD remain independent and unchanged.
